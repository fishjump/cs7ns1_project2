...
def create_model(captcha_length, input_shape, model_depth=5, module_size=2):
    ...
    x = keras.layers.Dense(
        captcha_length, activation='softmax', name='num_of_ch'
    )(x)
...
class ImageSequence(keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        y = numpy.zeros((self.batch_size, self.captcha_length),
                        dtype=numpy.uint8)
        ...
        for i in range(self.batch_size):
            ...
            y[i, :] = 0
            y[i, captcha_info['captcha_len'] - 1] = 1
...
def main(args=None):
    ...
    with tf.device('/device:GPU:0'):
        ...
        training_data = ImageSequence(
            args.train_dataset, args.batch_size, args.length, args.width, args.height)
        validation_data = ImageSequence(
            args.validate_dataset, args.batch_size, args.length, args.width, args.height)
